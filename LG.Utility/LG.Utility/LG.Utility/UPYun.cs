...
namespace LG.Utility
{
    ...
    {
        private const string _spaceName = "fanhuan";
        private const string _operator = "robot";
        private const string _password = "IAm@DiligentRobot20120910";
        ...
        public static bool UploadIamge(Stream inputStream, string location)
        {
            ...
            //_upyun.setContentMD5(UpYun.md5_file("Hello, this is a fucking image."));
        ...
        /// <param name="location">位置（如：/a/b/c.jpg）</param>
        public static bool ReloadloadIamge(string imageUrl, out string Msg)
        {
            Msg = "";
            UpYun upyun = new UpYun(_spaceName, _operator, _password);
            Hashtable headers = new Hashtable();
    ...
    public class UpYun
    {
        ...
        // 刷新缓存所用的api接口地址
        private string reloadFile_api_domain = "http://purge.upyun.com/purge/";
        ...
        */
        ...
        private void upyunAuthForReloadFile(Hashtable headers, string url, HttpWebRequest request)
        {
            DateTime dt = DateTime.UtcNow;
            string date = dt.ToString("ddd, dd MMM yyyy HH':'mm':'ss 'GMT'", CultureInfo.CreateSpecificCulture("en-US"));
            request.Date = dt;
            string auth = md5(url + "&" + this.bucketname + "&" + date + "&" + md5(this.password));
            headers.Add("Authorization", "UpYun " + this.bucketname + ":" + this.username + ":" + auth);
        }
        ...
        public bool reloadFileWorker(string method, string url, Hashtable headers, out string Msg,int requestTimeOut=3000)
        {
            ...
            // 签名
            upyunAuthForReloadFile(headers, url, request);
